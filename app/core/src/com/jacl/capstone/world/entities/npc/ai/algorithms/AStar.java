...
public class AStar
{
	...
	public IntArray getPath(int startX, int startY, int targetX, int targetY)
	{
		...
		while(open.size > 0){
			...
			if(x < lastColumn){
				...
				if(y < lastRow)
					addNode(node, x + 1, y + 1, 14); // Diagonals cost more, roughly
	...
	private void addNode(PathNode parent, int x, int y, int cost)
	{
		...
		if(node != null && node.runID == runID){ // Node already encountered for
																// this run.
			if(node.closedID != runID && pathCost < node.pathCost){ // Node isn't
																						// closed and
																						// new cost is
																						// lower.
